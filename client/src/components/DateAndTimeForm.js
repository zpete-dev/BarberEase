...
const DateAndTimeForm = ({ providers, sessionToken, setShowErrorPopup, selectedServices, selectedProviders, selectedTime, setSelectedTime, selectedDate, setSelectedDate }) => {
    ...
    useEffect(() => {
        const fetchAvailability = async () => {
            ...
            /*for (const asdfa of fullAvailability[selectedDateMST]) {
                console.log(asdfa)
            };
    ...
    function convertTimeStringToDate(timeString) {
        // Get today's date
        const today = DateTime.now().setZone('America/Denver').toJSDate();
        ...
        // Create a new Date object with the current date and time extracted from the string
        const date = new Date(today.getFullYear(), today.getMonth(), today.getDate(), hours, minutes);
        return date;
    }

    const removeInvalidTimeButtons = (aTimeButtonArray, aSelectedDate) => {
        const shopLocalTime = DateTime.now().setZone('America/Denver').toJSDate();
        //console.log(shopLocalTime);
        if(shopLocalTime.toISOString().split('T')[0] === aSelectedDate.split('T')[0]){
            const myTrimmedTimeButtonArray = [];
            for (const aTimeSlot of aTimeButtonArray){
                if(convertTimeStringToDate(aTimeSlot) > shopLocalTime){
                    myTrimmedTimeButtonArray.push(aTimeSlot);
                }
            }
            return myTrimmedTimeButtonArray;
        }
        return aTimeButtonArray;
    ...
    const setTimeButtonsByTimeArray = (availabilityArray, aSelectedDate) => {

        const availabilityForTheDay = availabilityArray[aSelectedDate];
        ...
        const myTimesForSelectedDateTrimmed = removeInvalidTimeButtons(myTimesForSelectedDate, aSelectedDate);
        const sortedTimes = myTimesForSelectedDateTrimmed.sort((a, b) => DateTime.fromFormat(a, "h:mm a").toMillis() - DateTime.fromFormat(b, "h:mm a").toMillis());
